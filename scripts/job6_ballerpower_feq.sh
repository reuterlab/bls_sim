...
pstar=$(awk -v ps=$pstar 'BEGIN{ps=sprintf("%.6f", ps); print ps}')
...
Rscript ballerpower.R $BALOUTDIR/${seltype}_N$((Ne/1000))k_r${rec}_grid${grid}_m${mut}_100/all_s${s1}-${s2}_h${h}_c${selage}_spl100_fixpstar.B2 $BALOUTDIR/neutralnull_N$((Ne/1000))k_r${rec}_grid0_m${mut}/all_s0-0_h0_c${selage}_spl100_fixpstar${pstar}.B2 $seltype $Ne $rec $mut $s1 $s2 $h $selage
