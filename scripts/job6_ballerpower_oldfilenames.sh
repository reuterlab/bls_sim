...
if [[ $seltype == +(OD) ]]; then
    Rscript ballerpower.R $BALOUTDIR/$SIMPREF/all_s${s1}-${s2}_c${selage}.B2 $BALOUTDIR/neutral_N$((Ne/1000))k_r${rec}_fornull/all_c${selage}.B2 $seltype $Ne $rec $mut $s1 $s2 $h $selage
elif [[ $seltype == +(AP|SA) ]]; then 
    Rscript ballerpower.R $BALOUTDIR/$SIMPREF/all_s${s1}-${s2}_h${h}_c${selage}.B2 $BALOUTDIR/neutral_N$((Ne/1000))k_r${rec}_fornull/all_c${selage}.B2 $seltype $Ne $rec $mut $s1 $s2 $h $selage
else
    exit 1
fi
