library(ggplot2)
library(cowplot)
pstarfile <- "Results/Predicted_power_plot_data_pstar.csv"
datapstar <- read.csv(pstarfile)
alphafile <- "Results/Predicted_power_plot_data_selalpha.csv"
...
g <- plot_grid(p+guides(shape=guide_legend(order=2)), a+guides(shape=guide_legend(order=2))+ylab(""),
          labels=c("A", "B"), ncol=2)
ggsave("Results/Predicted_power_pstar_alpha_panels.png", h=3.5, w=10, plot=g)
