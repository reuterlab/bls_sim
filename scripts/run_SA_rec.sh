...
for s1 in 0.01 0.02 0.05 0.09 0.1; do
    for s2 in 0.01 0.02 0.05 0.09 0.1; do
        for h in 0.5 0.25; do
            #qsub -v s1=$s1,s2=$s2,h=$h,rand_inv=1,OUTDIR='/SAN/reuterlab/balsel_detection/bls_sim/slimout/SA_N2k_r1e-6_grid0.01/' -t 1-$rep job_run_SA_N2k_r1e-6.sh
            ...
            #qsub -v s1=$s1,s2=$s2,h=$h,rand_inv=1,OUTDIR='/SAN/reuterlab/balsel_detection/bls_sim/slimout/SA_N20k_r1e-6_grid0.01/' -t 1-$rep job_run_SA_N20k_r1e-6.sh
...
for s1 in 0.1 0.2 0.5 0.9 1; do
    for s2 in 0.1 0.2 0.5 0.9 1; do
        for h in 0.5 0.25; do
            #qsub -v s1=$s1,s2=$s2,h=$h,rand_inv=1,OUTDIR='/SAN/reuterlab/balsel_detection/bls_sim/slimout/SA_N2k_r1e-6_grid0.1/' -t 1-$rep job_run_SA_N2k_r1e-6.sh
            ...
            #qsub -v s1=$s1,s2=$s2,h=$h,rand_inv=1,OUTDIR='/SAN/reuterlab/balsel_detection/bls_sim/slimout/SA_N20k_r1e-6_grid0.1/' -t 1-$rep job_run_SA_N20k_r1e-6.sh
