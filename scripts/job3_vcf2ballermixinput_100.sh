...
# copy over to scratch the set of 100 vcf files combining original sim outputs with extras
while read FNAME; do
    cp $FNAME $scratchdir
done < $PROJDIR/vcf/$SIMPREF'_m'$mut'_100'/output_s${s1}-${s2}_h${h}_c${selage}_fnames.txt
